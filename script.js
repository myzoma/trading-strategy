class TradingStrategy {
    ...
    async fetchTickers() {
        ...
        // في التطبيق الحقيقي، ستحتاج لاستخدام مفاتيح API الحقيقية
    ...
    filterCoins(tickers) {
        return tickers.filter(ticker => {
            ...
            if (CONFIG.EXCLUDED_COINS.includes(symbol)) return false;
            
            // التأكد من أن العملة مرتبطة بـ USDT
            if (!ticker.instId.endsWith('-USDT')) return false;
            
            // استبعاد العملات الصفرية
            if (price < CONFIG.MIN_PRICE) return false;
            
            // استبعاد العملات ذات الحجم المنخفض
    ...
    async analyzeCoin(ticker) {
      ...
      async calculateIndicators(ticker) {
        // محاكاة حساب المؤشرات الفنية
        // في التطبيق الحقيقي، ستحتاج لجلب بيانات الشموع وحساب المؤشرات الفعلية
    ...
    // دالة لحساب المؤشرات الفنية الحقيقية (للاستخدام مع بيانات حقيقية)
    ...
    findSupportResistance(highs, lows, period = 20) {
        ...
        for (let i = period; i < highs.length - period; i++) {
            ...
            if (isResistance) resistances.push({ index: i, price: highs[i] });
            if (isSupport) supports.push({ index: i, price: lows[i] });
        }
    ...
    validateCoinData(coin) {
        const requiredFields = ['symbol', 'price', 'volume24h', 'priceChange'];
